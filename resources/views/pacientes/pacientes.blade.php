    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4">{{ \Carbon\Carbon::parse($paciente->fecha_nacimiento)->format('Y-m-d') }}</td>                <td class="px-6 py-4">
                    ...
                    <form action="{{ route('pacientes.eliminar', $paciente->id) }}" method="POST" style="display:inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class=" text-red-600 dark:text-red-500 hover:underline" onclick="return confirm('¿Estás seguro de que deseas eliminar a este paciente?')"><ion-icon name="trash-outline"></ion-icon> Eliminar</button>
                    </form>
                </td>
            </tr>
            @endforeach
