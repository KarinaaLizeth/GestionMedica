    ...
    <div class="flex items-center justify-between flex-wrap md:flex-nowrap space-y-4 md:space-y-0 mb-4">
        <div class="flex items-center space-x-4">
            ...
            <div class="relative">
                ...
                <input type="text" id="table-search-users" class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Buscar colaborador">
            </div>
        </div>
        <div class="flex items-center">
            <a href="{{ route('colaboradores.crear') }}" class="text-white focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" style="background-color: #247b7b; transition: background-color 0.3s;" onmouseover="this.style.backgroundColor='#83c5be'"  onmouseout="this.style.backgroundColor='#247b7b'">
    ...
    <table id="example" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                <td class="px-6 py-4">{{ $colaborador->nombres }}</td>
                <td class="px-6 py-4">{{ $colaborador->apellidos }}</td>
                <td class="px-6 py-4">{{ $colaborador->correo }}</td>
                <td class="px-6 py-4">{{ $colaborador->telefono }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    function confirmDelete(colaboradorId) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('delete-form-' + colaboradorId).submit();
                swalWithBootstrapButtons.fire(
                    '¡Eliminado!',
