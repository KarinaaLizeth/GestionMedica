...
<nav x-data="{ open: false }" class="navbar from-blue-500 via-blue-600 to-blue-700 text-black shadow-lg">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="-mr-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-black hover:bg-blue-700 focus:outline-none focus:bg-blue-700 focus:text-gray-300 transition duration-150 ease-in-out">
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-blackhover:bg-blue-700">
                {{ __('Dashboard') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('doctores.index')" :active="request()->routeIs('doctores.index')" class="text-blackhover:bg-blue-700">
                {{ __('Doctores') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('doctores.crear')" :active="request()->routeIs('doctores.crear')" class="text-blackhover:bg-blue-700">
                {{ __('Agregar Doctor') }}
            </x-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('profile.edit')" class="text-blackhover:bg-blue-700">
                    {{ __('Profile') }}
                </x-responsive-nav-link>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-responsive-nav-link :href="route('logout')" class="text-blackhover:bg-blue-700" onclick="event.preventDefault(); this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
                </form>
            </div>
